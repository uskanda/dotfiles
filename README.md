...
### リポジトリの文脈注入（SessionStart フック）は uskn-harness へ移管

かつて `dot_local/bin/executable_claude-hosting-hook` が `settings.json` の `hooks.SessionStart` から
GitHub / GitLab の判定を注入していたが、2026-09 に [uskn-harness](https://github.com/uskanda/uskn-harness) の
Claude Code プラグイン（`~/.claude/skills/uskn-harness`、`uskn-harness sync` が symlink を張る）へ移した。
現在は hosting に加えてブランチモデル（default / integration / qa）も `<repo-context>` として注入される。
git ワークフロー系スキル（commit / push / pr / release など）も同じくハーネス側に移り、
このリポジトリが持つスキルは `chezmoi-merge` / `cleanup` / `set-workspace-theme` / `sync-claude-settings` だけ。
ハーネス由来の symlink やコピーは `.chezmoiignore` で除外しているので `chezmoi add ~/.claude` に巻き込まれない。
