...
- **ペインの環境変数は生成時点で凍る。** `ssh -X` で入り直しても、開きっぱなしのペインの `DISPLAY` は古い値のまま（tmux サーバを `-X` なしの `ssh` で立てていれば空）で、X アプリが `cannot open display` になる。`update-environment` が更新するのは attach 時の**セッション環境**だけで、既に走っているペインのシェルには届かない。[dot_config/zshrc](dot_config/zshrc) の `tmux-refresh-env`（`preexec` フック）が `DISPLAY` / `XAUTHORITY` / `SSH_AUTH_SOCK` / `SSH_CONNECTION` をコマンドの実行直前に引き直してこれを吸収している。**`precmd` に移さないこと** — precmd はコマンドが終わってから走るので attach 直後の 1 発目が古い環境のままになる（空 Enter は `accept-line-or-ls` が `zle reset-prompt` を呼ぶだけなので precmd も走らない）。**引き直す変数を増やすときは `WEZTERM_*` を入れないこと** — [ssh-window.sh](dot_config/shell/ssh-window.sh) が `wezterm cli` を叩く直前に自前で引き直しており、WSL 経路では Windows 側の値が来るので二重管理が噛み合わなくなる。詳細と実測値は [README.md](README.md) の「ペインの環境変数を attach ごとに引き直す」。
...
ターミナルエミュレータは WezTerm。設定は [dot_wezterm.lua](dot_wezterm.lua) 1 本で、`wezterm.target_triple` を見て macOS / Windows を分岐する（chezmoi は 1 ソース → 1 宛先なので、`.chezmoitemplates` ではなくファイル内分岐で解決している）。

> ⚠️ **macOS では `unix_domains` / `default_gui_startup_args = {'connect','unix'}` を復活させないこと。** GUI が mux のクライアントになると、✗ でウィンドウを 1 枚閉じたときに WezTerm が「リモートセッションを殺さないための domain 全体の detach」を行い、**その mux の全ウィンドウが消える**（GUI ログの `detaching domain` → `domain detached panes: [18 個…]` が証拠。detach だけを止める設定は無い）。加えて自己接続によるペイン増殖・`kill-pane` で消せないゴースト・GUI より長生きする mux へのゴミ蓄積が起きる。unix domain の元々の狙いは tmux ペインから `wezterm cli` を刺すことだが、macOS では tmux を使っていないので当てはまらない。tmux を使う Linux ネイティブ用の分岐だけ残してある。Windows では `default_domain` を WSL domain に向けてあり、PowerShell は `Ctrl+Shift+D` のランチャか `wezterm start --domain local -- powershell.exe` で出す。
...
- **macOS / Linux では `wezterm cli spawn` に `--domain-name` を渡さないこと。** `unix` domain のペインでは `WEZTERM_UNIX_SOCKET` が mux サーバ自身を指すため、`--domain-name unix` を渡すと mux が自分自身へ接続しにいき、ウィンドウが増殖する（実測: spawn 1 回で pane id が 2 → 7 まで飛び、`cli list` に `kill-pane` でも消せないゴーストが残り、GUI 接続時に一斉に開く。GUI 不在の mux に対しては spawn が返ってこない）。domain を省けば「今いるサーバの既定 domain」に 1 枚だけ開く。**WSL だけは逆**で、`--domain-name WSL:<distro>` を外すと Windows の `ssh.exe` が起動するため指定が必須。`__ssh_window_domain` は WSL のときだけ非空にしてある。溜まったゴーストは mux を落とすまで消えない（`pkill -f wezterm-mux-server`、SIGTERM で落ちる。GUI も道連れ）。
- **ウィンドウが増える経路には歯止めを残すこと。** 疎通できないホストへの ssh は、ssh 自身がタイムアウトまで黙って粘る一方でラッパーは即座にプロンプトへ戻るため、連打や再試行ループのぶんだけウィンドウが積み上がる。しかも mux（`unix` domain / WSL domain）は GUI を閉じても生き残るので、溜まったウィンドウは次に GUI が繋いだ瞬間に一斉に開き直す（実測: 失敗した ssh ウィンドウが 8 枚 mux に residual していた）。現在は ① 同じ宛先のウィンドウが生きている間は開き直さず既存を前面に出す（`SSH_WINDOW_DUP=1` で明示的に 2 枚目を開ける。`[ssh-window:failed]` マーカーが出ているキー待ちウィンドウは閉じてから開き直す＝失敗を繰り返しても宛先あたり 1 枚）② 生存ウィンドウが `SSH_WINDOW_MAX`（既定 8）に達したらその場実行に落とす ③ 失敗時のキー待ちを 5 分で打ち切る、の 3 段で止めている。**「n 秒以内の再実行だけ抑止する」形にしないこと** — 人間の叩き直しは数秒〜十数秒間隔なので短いクールダウンは素通りし、7 秒間隔の 3 連打で 3 枚開くのを実測した。台帳は `$XDG_RUNTIME_DIR`（無ければ `$TMPDIR`）の `ssh-window-<uid>.panes`。`wezterm cli list` が引けないときは「開いてよい」側に倒す（ガードのせいで ssh が開けなくなる方が困るため）。
...
- VSCode の `settings.json` は 3 OS 分のラッパ + 共有テンプレート構成。端末ごとに有無が変わる値（docker / im-select / 社用証明書）は `lookPath` と `stat` で存在確認してから出力し、無ければキーごと省略する。詳細は [README.md](README.md) の「VSCode ユーザー設定」。
  > ⚠️ **「全ウィンドウが黄色いタイトルバーになった」は VSCode のバグではない。** `window.title` / `workbench.colorTheme` / `workbench.colorCustomizations` は window スコープなので、workspace 設定に置いた値はそのウィンドウにしか効かない。全ウィンドウに出ているなら **user settings に混入している**。混入元だった金色 `#C79400` の `titleBar.*` / `activityBar.*` は 2 度再発したので [.vscode/settings.json](.vscode/settings.json) から**削除済み**（識別は `workbench.colorTheme` の Kimbie Dark と `window.title` の ⚙️ だけで付ける）。**ここに `workbench.colorCustomizations` を復活させないこと。** chezmoi が配る `workbench.colorCustomizations` は `statusBar.*`（`#005f5f`）の 5 キーだけなので、`chezmoi diff <settings.json のパス>` で混入は一目で分かる。復旧は混入した端末すべてで `chezmoi apply --force`。`settingsSync.ignoredSettings` に `workbench.colorCustomizations` / `window.title` を入れて Sync 経由の伝播は止めてあるが（再発時の sync スナップショットに金色は 1 件も無く、混入はその端末での直接書き込みだった）、**既に各端末に残った値は消えない**。手順は [README.md](README.md) の「全ウィンドウが同じ色／同じタイトルになったとき」。
