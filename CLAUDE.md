...
- **ペインの環境変数は生成時点で凍る。** `ssh -X` で入り直しても、開きっぱなしのペインの `DISPLAY` は古い値のまま（tmux サーバを `-X` なしの `ssh` で立てていれば空）で、X アプリが `cannot open display` になる。`update-environment` が更新するのは attach 時の**セッション環境**だけで、既に走っているペインのシェルには届かない。[dot_config/zshrc](dot_config/zshrc) の `tmux-refresh-env`（`precmd` フック）が `DISPLAY` / `XAUTHORITY` / `SSH_AUTH_SOCK` / `SSH_CONNECTION` をプロンプトごとに引き直してこれを吸収している。**引き直す変数を増やすときは `WEZTERM_*` を入れないこと** — [ssh-window.sh](dot_config/shell/ssh-window.sh) が `wezterm cli` を叩く直前に自前で引き直しており、WSL 経路では Windows 側の値が来るので二重管理が噛み合わなくなる。詳細と実測値は [README.md](README.md) の「ペインの環境変数を attach ごとに引き直す」。
