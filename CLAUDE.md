...
- VSCode の `settings.json` は 3 OS 分のラッパ + 共有テンプレート構成。端末ごとに有無が変わる値（docker / im-select / 社用証明書）は `lookPath` と `stat` で存在確認してから出力し、無ければキーごと省略する。詳細は [README.md](README.md) の「VSCode ユーザー設定」。
  > ⚠️ **「全ウィンドウが黄色いタイトルバーになった」は VSCode のバグではない。** `window.title` / `workbench.colorTheme` / `workbench.colorCustomizations` は window スコープなので、[.vscode/settings.json](.vscode/settings.json)（dotfiles ウィンドウ識別用の金色 `#C79400`）に置いた値はそのウィンドウにしか効かない。全ウィンドウに出ているなら **user settings に混入している**。chezmoi が配る `workbench.colorCustomizations` は `statusBar.*`（`#005f5f`）の 5 キーだけなので、`chezmoi diff <settings.json のパス>` で一目で分かる。復旧は混入した端末すべてで `chezmoi apply --force`。`settingsSync.ignoredSettings` に `workbench.colorCustomizations` / `window.title` を入れて以後の伝播は止めてあるが、**既に各端末に残った値は消えない**。手順は [README.md](README.md) の「全ウィンドウが同じ色／同じタイトルになったとき」。
